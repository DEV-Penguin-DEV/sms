...
$(document).ready(function () {
    ...
    let list = $(".js-example-basic-single").select2({
        // matcher: matchStart,
        closeOnSelect: false,
        multiple: true,
    ...
    $('.intro__filter').each(function (i, e) {
        ...
        if (count) {
            $('<i>' + count + '</i>').appendTo($(this).find('.select2-selection__choice__display'))
        }
    })
    ...
    $('body').on('click', '.select2-results__option', function (e) {
        openSearch = false;
        let index = parseInt($(this).parents('.select2-container.select2-container--default.select2-container--open').attr('data-id'));
        $('.intro__filter:nth-child(' + (index + 1) + ')').removeClass('open');
        $(this).parents('.select2-container--open').removeClass('open');
        let indexOption = 1;
    ...
    let indSelect = 0;
    $('body>span.select2-container.select2-container--default.select2-container--open').each(function (i, e) {
        $(this).addClass('inselect-' + indSelect);
        $(this).attr('data-id', indSelect);
        indSelect++;
    })

    $('.intro__filter>svg').on('click', function (e) {
        showTable(true);
        // if(!openSearch) {
        let index = $(this).parents('.intro__filter').index();
        let el = $('body>span.select2-container.select2-container--default.select2-container--open.inselect-' + index);
        if ($(el).hasClass('open')) {
            $(this).parents('.intro__filter').removeClass('open');
            $(el).removeClass('open');
        } else {
            $(this).parents('.intro__filter').addClass('open');
            $(el).addClass('open');
        }
        // }
    })
    ...
    $('.select2-search textarea').on('click', function (e) {
        openSearch = true;
        $(this).parents('.intro__filter').addClass('open');
    })

    $('.intro__head div').on('click', function (e) {
        $('.intro__head div').removeClass('active');
    ...
    $('.intro__list--main .intro__item').on('click', function (e) {
        e.preventDefault();
        $(this).toggleClass('active');
    })

    $('.intro__favorite-button').on('click', function (e) {
        e.preventDefault();
        $(this).toggleClass('active');
    })
    ...
    $(window).on('scroll', function (event) {
        ...
        $('.needAnim').each(function (e) {
            ...
            if (current > top) {
                ...
                if (activeSection === 3 && $(this).hasClass('advantage__item-2')) {
                    $('.advantage__item').removeClass('animated');
                }
            } else {
        ...
        // if(activeSection != 1){
        //     for(var i = 1; i<activeSection; i++){
        //         $('section:nth-child('+i+') .needAnim').removeClass('animated');
        //     }
        // }

        if ($('section').hasClass("intro")) {
            var offset = $(".intro").offset();
            let offsetHeight = $(".intro+section").height() + 300;
            if ($(this).scrollTop() >= offset.top && $(this).scrollTop() <= (offset.top + offsetHeight)) {
                let percent = (($(this).scrollTop() - offset.top) / offsetHeight) * 100;
                $('.intro__title img').css('transform', 'translateY(' + percent + 'px)');
            }
        }

        if ($('section').hasClass("activation")) {
            var offset = $(".activation").offset();
            let offsetHeight = $(".activation+section").height() + 300;
            if ($(this).scrollTop() >= offset.top && $(this).scrollTop() <= (offset.top + offsetHeight)) {
                let percent = (($(this).scrollTop() - offset.top) / offsetHeight) * 100;
                $('.activation__question').css('transform', 'translateY(' + percent + 'px)');
            };
        }
    ...
    $('.advantage__item, .advantage__item-2 ').mousemove(function (event) {
        var left = event.pageX - $(this).offset().left;
        var top = event.pageY - $(this).offset().top;
        let width = $(this).width();
        let height = $(this).height();
        let x = Math.floor((width / 2 - left) / width * 100) / 3;
        let y = Math.floor((height / 2 - top) / width * 100) / 4;
        if ($(window).width() > 1024) {
            $(this).find('img').animate({ left: x, top: y }, 0);
        }
    });
    ...
    setTimeout(() => {
        textSplit('.intro__subtitle', 200);
    }, 1000)


    function textSplit(clases, delay) {
        let str = $(clases).attr('data-text').split('');
        $(clases).html('');
        ...
        $.each(str, function () {
            setTimeout(() => {
                // console.log('test');
                $(clases).append('<i class="splitting">' + this + '</i>');
            }, delay * i);
            i++;
        });
        ...
        setTimeout(() => {
            let t = 0;
            $($(clases + '>i').get().reverse()).each(function () {
                setTimeout(() => {
                    $(this).remove();
                }, 50 * t);
                t++;
    ...
    function showTable(bool) {
        $('.intro').addClass('hide');
        let iW = $('.intro__main').attr('width');
        let iH = $('.intro__main').attr('height');
        let vw = $(window).width();
        ...
        $('.intro__main').css('left', -iW + vw);
        $('.intro__main').css('bottom', -iH + vh);

        if (bool) {
            let int = setInterval(() => {
                let top = $('.intro__filters').offset().top;
                $('body>span.select2-container.select2-container--default.select2-container--open').css('top', top + 50);
            }, 10);
            setTimeout(() => {
                let top = $('.intro__filters').offset().top;
                $('body>span.select2-container.select2-container--default.select2-container--open').css('top', top + 50);
                $('.intro').addClass('stopTransition');
                clearInterval(int);
                $('.intro__exit').addClass('active');
            }, 3000)
        } else {
            let top = $('.intro__filters').offset().top;
            $('body>span.select2-container.select2-container--default.select2-container--open').css('top', top + 50);
    ...
    $('.intro__exit').on('click', function (e) {
        e.preventDefault();
        $(this).removeClass('active');
        $('.intro').removeClass('stopTransition');
        setTimeout(() => {
            $('.intro').addClass('reverse');
            $('.intro').removeClass('hide');
            setTimeout(() => {
                $('.intro__main').removeAttr('style');
                $('.intro').removeClass('reverse');
            }, 3000)
        }, 100)
    ...
    jQuery('.advantage__list').swipe({
        swipeStatus: function (event, phase, direction, distance, duration, fingerCount, fingerData, currentDirection) {
            if ($(window).width() < 1200) {
                let activeIndex = $('.advantage__nav div.active').index() + 1;
                $('.advantage__list').addClass('noTransition');
                if (phase == "end") {
                    //сработает через 20 пикселей то число которое выбрали в threshold
                    if (direction == 'left') {
                        addNavActive('.advantage__nav', activeIndex, true);
                        let element = $('.advantage__list>div:nth-child(1)');
                        let width = parseInt(element.css('width').split('px')[0]) + 40;
                        $(element).animate({ 'margin-left': '-' + width + 'px' }, 300, function () {
                            $(this).appendTo('.advantage__list').css('margin-left', '0px');
                        });
                    }
                    if (direction == 'right') {
                        addNavActive('.advantage__nav', activeIndex, false);
                        let element = $('.advantage__list>div:last-child');
                        let width = parseInt(element.css('width').split('px')[0]) + 40;
                        $(element).css('margin-left', '-' + width + 'px').prependTo('.advantage__list').animate({ 'margin-left': '0px' }, 300);
    ...
    jQuery('.advantage__list-2').swipe({
        swipeStatus: function (event, phase, direction, distance, duration, fingerCount, fingerData, currentDirection) {
            if ($(window).width() < 1200) {
                let activeIndex = $('.advantage__nav-2 div.active').index() + 1;
                $('.advantage__list-2').addClass('noTransition');
                if (phase == "end") {
                    //сработает через 20 пикселей то число которое выбрали в threshold
                    if (direction == 'left') {
                        addNavActive('.advantage__nav-2', activeIndex, true);
                        let element = $('.advantage__list-2>div:nth-child(1)');
                        let width = parseInt(element.css('width').split('px')[0]) + 40;
                        $(element).animate({ 'margin-left': '-' + width + 'px' }, 300, function () {
                            $(this).appendTo('.advantage__list-2').css('margin-left', '0px');
                        });
                    }
                    if (direction == 'right') {
                        addNavActive('.advantage__nav-2', activeIndex, false);
                        let element = $('.advantage__list-2>div:last-child');
                        let width = parseInt(element.css('width').split('px')[0]) + 40;
                        $(element).css('margin-left', '-' + width + 'px').prependTo('.advantage__list-2').animate({ 'margin-left': '0px' }, 300);
    ...
    function addNavActive(parent, index, direction) {
        let length = $(parent).find('div').length;
        $(parent).find('div').removeClass('active');
        if (direction) {
            if (index + 1 <= length) {
                $(parent).find('div:nth-child(' + (index + 1) + ')').addClass('active');
            } else {
                $(parent).find('div:first-child').addClass('active');
            }
        } else {
            if (index - 1 >= 1) {
                $(parent).find('div:nth-child(' + (index - 1) + ')').addClass('active');
            } else {
                $(parent).find('div:last-child').addClass('active');
            }
        }
